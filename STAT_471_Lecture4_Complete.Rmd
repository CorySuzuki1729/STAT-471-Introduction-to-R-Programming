...
Note that since we have feature names and we want read.cv to recognize the first row, we need to set the argument "header=TRUE".
...
```{r}
ggplot(imputed_data, aes(x = "dollars", y = Barnala)) +
  geom_boxplot()
```
...
ggplot(punjab_long, aes(x = District, y = Value)) +
  ...
  labs(title = "Boxplots of District Values (Columns 3–22)",
       x = "District",
       y = "Value") +
  theme_minimal() +
