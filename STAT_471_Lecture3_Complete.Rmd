...
bisection_alg = function(f, a, b, tol, max_iter) {
  if (f(a) * f(b) >= 0) {
    stop("Function does not have opposite signs at endpoints")
  }
  iter = 0
...
newton_alg = function(fun, funpr, x0, tol, iter) {
  ...
  stop("Algorithm reached max iterations")
}
```
...
```{r}
curve(f, from = -3, to = 3, col = "blue", lwd = 2,
      xlab = "x", ylab = "f(x)", main = "Graph of f(x) = x^2 - 2")
...
```{r}
f = function(x) x^3 -3*x^2 + (4/3)

curve(f, from = 0, to = 1, col = "blue", lwd = 2,
      xlab = "x", ylab = "f(x)", main = "Graph of f(x) = x^3 -3*x^2 + (4/3)")
abline(h = 0, col = "red", lty = 2)
```

```{r}
f = function(x) x^3 -3*x^2 + (4/3)
fprime = function(x) 3*x^2 - 6*x
...
#rbinom is used to generate random variables from the binomial distribution.To draw from Bernoulli, we set size to 1. 

x = rbinom(20, size=1, prob=0.6)

#Let's try this with 20 Bernoulli random variables with success probability p=0.6.

#Log-likelihood derivative (score)
...
#Now let's call the Newton-Raphson MLE algorithm.
...
# Second derivative
info = function(p, data) {
  (-n/p^2) - ((sum(data) - length(data)) / (1-p)^2)
}
